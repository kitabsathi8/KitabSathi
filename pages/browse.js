...
const DISTRICTS = ["All Nepal","Kathmandu","Lalitpur","Bhaktapur","Pokhara","Chitwan","Butwal","Biratnagar","Dharan","Birgunj"];
const CONDITIONS = ["Like New","Good","Fair","Damaged"];

function condColor(c){ if(c==="Like New"||c==="Good") return C.condGood; if(c==="Fair") return C.condFair; return C.condDamaged; }
...
function getEmoji(category,subject){
  ...
  if(s.includes("science")) return "📕"; if(s.includes("math")) return "📘";
  if(s.includes("nepali")) return "📗"; if(s.includes("physics")) return "📕";
  if(s.includes("chemistry")) return "📗"; if(s.includes("account")) return "📊";
  return "📚";
}
...
function FilterTag({ label, onRemove }){
  return (
    <span style={{ background:"rgba(76,201,232,0.12)", color:C.primary, borderRadius:20, padding:"4px 10px", fontSize:12, fontWeight:600, display:"flex", alignItems:"center", gap:5 }}>
      {label}<button onClick={onRemove} style={{ background:"none", border:"none", color:C.primary, cursor:"pointer", fontSize:14, padding:0, lineHeight:1 }}>✕</button>
    </span>
  );
}
...
export default function BrowsePage(){
  ...
  useEffect(()=>{
    async function fetchBooks(){
      const { data, error } = await supabase.from("books").select("*").eq("sold",false).order("created_at",{ascending:false});
      if(error) setDbError(error.message);
      else setAllBooks(data||[]);
      setDbLoading(false);
    }
    fetchBooks();
  },[]);

  function toggleCondition(c){ setConditions(prev=>prev.includes(c)?prev.filter(x=>x!==c):[...prev,c]); }
  function resetFilters(){ setSearchQuery("");setSelectedGrade("All");setSelectedLocation("All Nepal");setConditions([]);setMinPrice("");setMaxPrice("");setSortBy("newest");setNegotiableOnly(false); }

  const filteredBooks = useMemo(()=>{
    let books = allBooks.filter(b=>{
      if(searchQuery.trim()){const q=searchQuery.toLowerCase();if(![b.title,b.author,b.subject,b.grade].some(f=>(f||"").toLowerCase().includes(q))) return false;}
  ...
  return (
    <>
      <Head>
        <title>Browse Books — KitabSathi Nepal</title>
        <meta name="description" content="Browse used academic and non-academic books for sale across Nepal on KitabSathi." />
      </Head>
        ...
        <div style={{ background:C.navBg, borderBottom:`1px solid ${C.border}`, padding:"14px 24px" }}>
          <div style={{ maxWidth:920, margin:"0 auto", display:"flex", gap:10 }}>
            <div style={{ flex:1, display:"flex", background:C.bg, border:`2px solid ${C.primary}`, borderRadius:10, padding:4 }}>
              <input type="text" placeholder="Search by title, author, subject, grade..." value={searchQuery} onChange={e=>setSearchQuery(e.target.value)}
          ...
          <main style={{ flex:1, minWidth:0 }}>
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:12 }}>
              <div style={{ fontSize:13, color:C.muted }}>
                {dbLoading?"Loading...":(<><span style={{ color:C.text, fontWeight:700 }}>{filteredBooks.length}</span> books found</>)}
              </div>
            ...
            {dbError&&(
              <div style={{ textAlign:"center", padding:"40px 20px", background:C.navBg, border:`1px solid ${C.border}`, borderRadius:12 }}>
                ...
                <div style={{ fontSize:14, color:C.muted }}>{dbError}</div>
              </div>
            )}
            ...
            {!dbLoading&&!dbError&&allBooks.length===0&&(
              <div style={{ textAlign:"center", padding:"60px 20px", background:C.navBg, border:`1px solid ${C.border}`, borderRadius:12 }}>
                ...
                <div style={{ fontSize:14, color:C.muted, marginBottom:20 }}>Be the first to list a book on KitabSathi.</div>
            ...
            {!dbLoading&&!dbError&&allBooks.length>0&&filteredBooks.length===0&&(
              <div style={{ textAlign:"center", padding:"60px 20px", background:C.navBg, border:`1px solid ${C.border}`, borderRadius:12 }}>
                ...
                <div style={{ fontSize:14, color:C.muted, marginBottom:20 }}>Try adjusting your filters</div>
                <button onClick={resetFilters} style={{ padding:"10px 24px", background:C.primary, color:C.primaryDark, border:"none", borderRadius:20, fontSize:14, fontWeight:700, cursor:"pointer" }}>Clear All Filters</button>
              </div>
            )}
          </main>
        </div>
