// pages/index.js — KitabSathi Homepage
...
function AnimatedCount({ value, suffix="" }) {
  ...
  useEffect(() => {
    if (!value) return;
    let current = 0;
    const steps = 50, interval = 1200 / steps, increment = value / steps;
...
const CATEGORIES = [
  { icon:"🏫", label:"Grade 1–5"    },
  { icon:"📐", label:"Grade 6–10"   },
  { icon:"⭐", label:"SEE Books"    },
  { icon:"🔬", label:"Grade 11–12"  },
  { icon:"🎓", label:"Bachelor"     },
  { icon:"📖", label:"Non-Academic" },
];
  ...
  return (
    <div style={{ background:C.cardBg, border:`1px solid ${C.border}`, borderRadius:12, padding:16, cursor:"pointer" }}>
      ...
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
        ...
        <button
          onClick={() => book.seller_phone && window.open(`https://wa.me/977${book.seller_phone}?text=Hi! I saw your book "${book.title}" on KitabSathi. Is it available?`)}
...
export default function Home() {
  ...
  const [recentBooks, setRecentBooks] = useState(SAMPLE_BOOKS);
  ...
  useEffect(() => {
    ...
    fetchData();
    const channel = supabase.channel("homepage-stats")
      ...
      <div style={{ minHeight:"100vh", background:C.bg, fontFamily:"system-ui, sans-serif" }}>
        ...
        <section style={{ background:C.heroBg, padding:"60px 24px 48px", textAlign:"center" }}>
          <div style={{ maxWidth:640, margin:"0 auto" }}>
            ...
            <div style={{ display:"flex", gap:6, maxWidth:520, margin:"0 auto 20px", background:C.navBg, borderRadius:12, border:`2px solid ${C.primary}`, padding:4 }}>
              <input type="text" placeholder="Search by title, author, grade, subject..." value={searchQuery} onChange={e=>setSearchQuery(e.target.value)}
                style={{ flex:1, border:"none", outline:"none", padding:"10px 12px", fontSize:15, borderRadius:8, background:"transparent", color:C.text }} />
              <button style={{ background:C.primary, color:C.primaryDark, border:"none", borderRadius:8, padding:"10px 20px", fontWeight:700, cursor:"pointer", fontSize:14 }}>Search</button>
            </div>
        ...
        <section style={{ background:C.navBg, borderTop:`1px solid ${C.border}`, padding:"40px 24px" }}>
          <div style={{ maxWidth:700, margin:"0 auto", textAlign:"center" }}>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"repeat(3, 1fr)", gap:24 }}>
              {[
                ["1","📝","List Your Book","Upload details, set your price — takes 2 minutes"],
            ...
            <a href="/sell" style={{ display:"inline-block", marginTop:28, padding:"12px 32px", background:C.primary, color:C.primaryDark, borderRadius:24, fontSize:14, fontWeight:700, textDecoration:"none" }}>
        ...
        <footer style={{ background:C.bg, borderTop:`1px solid ${C.border}`, padding:"20px 24px", textAlign:"center" }}>
          <div style={{ fontSize:12, color:C.muted }}>
            © 2025 KitabSathi · Made with ❤️ for Nepal ·{" "}
            <span style={{ color:C.primary }}>kitabsathi.com.np</span>
          </div>
        </footer>
      </div>
    </>
